...
class TscanTrainer(BaseTrainer):
    ...
    def valid(self, data_loader):
        ...
        self.model.eval()
        # self.model.train()
        print('validation in model.train mode:\n')
        valid_step = 0
        with torch.no_grad():
...
#                 print('pred_shape:', pred_ppg_test.shape)
#                 print('label_shape:', labels_test.shape)
#                 print('\n')
# # -----------------------------------------------------new------------------
#                 print('batch_size is::', batch_size)
                # if self.config.TEST.MC_DROPOUT:
                #     print('MC-DropOut Enabled in Testing')
                #     pre3 = self.mc_dropout(data_test, label= labels_test)
                # print('pred3_shape', pre3.shape)
                # print('\n')
# --------------------MC-drop----------------------------------------------------------
    ...
    def mc_dropout(self, data_test, label, num_samples = 100, save=False):
        preds = []
        self.model.train()
        with torch.no_grad():
        ...
        if save:
            ...
            df1.to_excel('/home/be-light/usman/results/predictions.xlsx', index=False)
            # df1.to_excel('/home/be-light/usman/results/predictions1.xlsx', index=True)
            df2.to_excel('/home/be-light/usman/results/labels.xlsx', index=False)
    ...
    #         else:
    #             m.eval()
    # def mc_dropout(self, x, num_samples =10):
    #     # model = dropout(model)
    #     # for m in model.modules():
    #     #     if m.__class__.__name__.startswith('Dropout'):
    #     #         m.train()
    ...
    #         predictionss.append(output.detach().cpu().numpy())

    #     pred = torch.tensor(predictionss)
    #     mean_pred = pred.mean(axis = 0)
    #     std_pred = pred.std(axis = 0)
        
    #     return mean_pred, std_pred, output
